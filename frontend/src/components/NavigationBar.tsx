            ...
            <Query query={GET_ALL_CATEGORIES}>
                {({loading, error, data}) => {
                    ...
                    return (
                        <div>
                            <nav
                                className="relative flex justify-between bg-white text-black w-full px-5 xl:px-12 py-6 z-20">
                                {/* left section */}
                                <div className="flex items-center space-x-5">
                                    <ul className="hidden md:flex px-4 font-light font-heading space-x-12">
                                        {data.categories.map((category, index) => (
                                            <li key={index}>
                                                <NavLink
                                                    to={`/${category.category_name.toLowerCase()}`}
                                                    className={this.props.pathName.startsWith(`/${category.category_name.toLowerCase()}`) ? 'text-green-500 pb-navigation-bottom mb-[-2px] border-b-2 border-green-500' : 'text-black'}
                                                    data-testid={this.props.pathName.startsWith(`/${category.category_name.toLowerCase()}`) ? 'active-category-link' : 'category-link'}
                                                >
                                ...
                                <div className="flex justify-center flex-1">
                                    <a href="#" className="font-heading pr-20">
                                        <svg width="33" height="31" viewBox="0 0 33 31" fill="none"
                                            ...
                                            <path
                                                ...
                                                fill="white"/>
                                            <defs>
                                                <linearGradient id="paint0_linear_150_362" x1="25.8733" y1="26.3337"
                                                                x2="7.51325" y2="4.9008" gradientUnits="userSpaceOnUse">
                                ...
                                <div className="group relative xl:flex items-center space-x-5">
                                    <a className="flex items-center hover:text-gray-200" href="#" onClick={(e) => {
                                        e.preventDefault();
